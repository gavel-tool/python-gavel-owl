...
def apply_mapping_to_element(element, name_dict):
    if element.__visit_name__ == "quantified_formula":
        ...
        predicate = remove_apostrophes(element.predicate)
        if predicate in name_dict:
            predicate = name_dict[predicate]
        return PredicateExpression(predicate, [apply_mapping_to_element(arg, name_dict) for arg in element.arguments])
    elif element.__visit_name__ == "functor_expression":
        functor = remove_apostrophes(element.functor)
        if functor in name_dict:
            functor = name_dict[functor]
        return FunctorExpression(functor, [apply_mapping_to_element(arg, name_dict) for arg in element.arguments])
    elif element.__visit_name__ == "constant":
        symbol = remove_apostrophes(element.symbol)
        if symbol in name_dict:
            symbol = name_dict[symbol]
...
def parse_tptp_files(filenames):
    files = []
    formulas = []
    for file in filenames:
        parser = TPTPParser()
        new_formulas = list(map(lambda x: x.formula, parser.parse_from_file(file)))
        files += [file for _ in range(len(new_formulas))]
...
def build_annotated_formulas(formulas, original_annotations):
    ...
    for formula in formulas:
        if len(original_annotations) > i:
            name, annotation = original_annotations[i]
        else:
            name, annotation = 'fol_axiom', ""
